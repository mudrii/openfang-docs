...
**Current stable release: v0.5.7** (2026-04-08)
...
### Added
- **Multi-instance Hands** -- Hands can now run multiple instances concurrently
- **Global fallback providers** -- Configure fallback LLM providers at the global level for automatic failover
- **Custom Hand persistence** -- Custom Hand configurations survive restarts

### Fixed
- Security fix: `rm` bypass (issue #919) patched
- Moonshot session repair -- broken sessions are now auto-recovered
- Discord heartbeat reliability improvements
- System prompt leak fix -- prevents prompt content from appearing in agent responses
- Version stamp fix -- correct version reported across all endpoints

### Security
- Removed `rm` command bypass vector (#919)
- System prompt leak prevention hardened
...
### Added
- **SSRF allowlist** -- Configurable allowlist for server-side request forgery protection
- **Expanded embedding detection** -- More embedding model patterns recognized automatically
- **Ollama 128K context** -- Ollama provider now supports 128K context windows

### Fixed
- Desktop version sync -- desktop app version now matches CLI version correctly
...
### Fixed
- Qwen `tool_calls` response parsing fix
- LINE webhook verification fix
- Local skill install from filesystem path
- `exec_policy` bypass patched
...
### Fixed
- Install script reliability fix
- Session endpoint error handling fix
- Agent timeout increased for long-running operations
- Model switching during active conversation fix
...
### Fixed (19 bug fixes)
- Groq deprecated model removal from catalog
- Ollama streaming response handling
- Gemini SSE (Server-Sent Events) framing fix
- Heartbeat now skips idle agents (no false positives)
- Hand polling interval increased to 3600s default
- `openfang doctor` diagnostic improvements
- Dashboard reactivity fixes for settings changes
- Streaming response reactivity improvements
- Multiple additional community-reported fixes
...
### Fixed (12 bug fixes)
- Subset of fixes later included in v0.5.3
...
### Fixed (9 community PRs)
- Dashboard loading state improvements
- KaTeX loaded on demand to prevent first-paint blocking
- Provider name normalization for consistent display
- Approval request visibility in dashboard
...
### Milestone Release (29 bug fixes, 6 new features)

### Added
- **Image generation pipeline** -- Agents can generate images via supported providers
- **WeCom (WeChat Work) adapter** -- Full channel adapter with inbound callbacks, outbound API, and access token caching
- **Docker sandbox runtime** -- Container-isolated execution environment for skills and tools
- **Shell skill runtime** -- New `shell` runtime type for Bash/sh skill scripts
- **Slack unfurl** -- Link preview support in Slack messages
- **`release-fast` build profile** -- Optimized release builds with LTO and codegen-units=1

### Fixed
- 29 community-reported bug fixes across all subsystems
...
### Fixed
- Image pipeline processing fix for edge cases
- Community troubleshooting FAQ added to documentation
...
### Fixed
- `HandCategory` enum deserialization fix
- LINE adapter token refresh
- `max_iterations` configuration now respected
- `knowledge_query` SQL injection prevention
- WhatsApp error message handling
- Dashboard provider list display

*Includes fixes from v0.4.5 through v0.4.7.*
...
### Fixed
- Bug fix batch (rolled into v0.4.8 release notes)
...
- **Shell skill runtime** -- Execute Bash/sh scripts as skills
- **Docker runtimes support** -- Container-based skill execution
- **`release-fast` build profile** -- Optimized production builds
...
- Slack unfurl links
- Agent rename persistence
- Async session save
- Codex `id_token` handling
- Channel agent re-resolution
- Chromium `--no-sandbox` for root users
- Tool error guidance injection
- OpenClaw provider alias migration compatibility
...
### Added
- Free model aliases: `free`, `openrouter/free`, `free-reasoning` for OpenRouter free models
- OpenSSL statically compiled (`libssl` vendored -- no runtime dependency)
- Config overwrite warning toast notification

### Fixed
- External links now open in new tabs in the web dashboard
- Browser Hand install detection improved
...
### Fixed
- LoopGuard poll detection now purely keyword-based (fewer false positives)
- Custom model provider now shows `provider:model` format
- Telegram typing indicator refreshes every 4 seconds
- Channel router "No agent selected" error on API-created agents
...
- Pre-emptive quota-aware token compaction
...
### Community Batch Release

### Added
- **Cron workflow execution** -- Scheduled workflow runs with atomic `try_claim_for_run`
- **Matrix adapter** -- Full Matrix protocol support as a channel adapter
- **Cross-channel defaults** -- Default agent and model settings that apply across all channels
- **Dashboard auth** -- Authentication for the web dashboard
- **Telegram reply context** -- Reply-to message context forwarded to agents
...
### Fixed (11 community issues)
- Agent tool filtering -- only declared tools sent to LLM (saves 15-20K tokens)
...
- Token quota defaulted to unlimited (0)
- Safe string slicing + desktop shutdown race condition
...
### Added
- **Trading Hand (8th bundled Hand)** -- Autonomous 8-phase trading pipeline with adversarial bull/bear debate, risk management gate, Alpaca API execution, and trade analytics

### Fixed
- Streaming `<think>` tag leak fixed with `StreamingThinkFilter`
- Cron jobs orphaned after agent deletion
...
- UTF-8 CJK boundary panic (9 affected sites)
...
### Added
- 6 OpenRouter free model variants
...
### Fixed
- Streaming empty text and 0 token counts
...
- **Shell execution rewrite** -- `shell_exec` now uses direct `execve()`, no shell interpreter, eliminating the shell injection vulnerability class entirely
- **Authentication hardening** -- All mutating endpoints (POST/PUT/DELETE) require auth; GET-only endpoints remain public
...
### Fixed
- UTF-8 panic fixes across the codebase
- `shell_exec` infinite loop prevention
- Stale Groq model removed from catalog
- System prompt cache invalidation
- Empty response handling for edge cases
...
*Pre-release versions (v0.3.24--v0.3.29) are rapid iteration builds from 2026-03-05 to 2026-03-08 without public release notes.*
